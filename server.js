...
// attach WS to the same HTTP server (so it uses Render's PORT)
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'signal':
        if (partner?.readyState === 1) send(partner, 'signal', { data: msg.data });
        break;
      case 'next':
        unpair(ws._id, false);
        enqueue(ws);
        if (partner?.readyState === 1) enqueue(partner);
        break;
      case 'bye':
        unpair(ws._id, true);
        break;
